...
vars :: Exp -> [IndexedVar]
vars (X var)= [var]  
vars (Lam var e)= union [var] (vars e)   
vars (App a b)= union (vars a) (vars b)   
...
freeVars (X var) = [var] 
freeVars (App a b ) = union (freeVars a) (freeVars b)
freeVars (Lam var e) = delete var  (freeVars e) 
...
occursFree var e = elem (var) (freeVars e) 
...
-- Gets the next index of the given variable.
getIndex name variables = (maximum $ [ivCount x | x<-variables, ivName x == name]) +1

freshVar :: IndexedVar -> [IndexedVar] -> IndexedVar
freshVar (var) (variables) 
 | elem var variables = IndexedVar (ivName var) ((getIndex (ivName var) variables ))
 | otherwise = var 
...
renameVar toReplace replacement (X variable) 
 | toReplace == variable = X replacement
 | otherwise = (X variable)

renameVar toReplace replacement (Lam variable e) 
 | toReplace == variable = Lam replacement (renameVar toReplace replacement e)
 | otherwise = Lam variable (renameVar toReplace replacement e) 
...
substitute toReplace replacement (X var) 
 | toReplace == var = replacement
 | otherwise = (X var)

substitute toReplace replacement (App a b)  = App (substitute toReplace replacement a) (substitute toReplace replacement b)
substitute toReplace replacement (Lam var e) 
 | var /= toReplace && (not $ occursFree (var) replacement) = (Lam var (substitute toReplace replacement e)) 
 | var /= toReplace && (occursFree (var) replacement) = (Lam (freshVar var (freeVars replacement))  (substitute toReplace replacement e)) 
 | otherwise = (Lam var e)
...
normalize :: Exp -> Exp
normalize (X p) = (X p) 
normalize (Lam a b) = (Lam a $ normalize b) 
normalize (App a b) = case left 
 of 
  (Lam x e) -> normalize (substitute x b e)
  _ -> App a b
 where 
  left =  normalize a
  right = normalize b 
